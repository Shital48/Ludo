    ...
    public class Dice : MonoBehaviour
    {
        ...
        IEnumerator RollTheDice()
        {
            ...
            for (int i = 0; i <= Constants.diceRollShuffles; i++)
            {
                ...
                Sound.PlaySound(Constants.Instance.sfxDiceRoll, MatchManager.Instance.audioSource);
            ...
            // Allow user to select a token and move that token 
            if (testToken)
                testToken.Move(MatchManager.DiceResult);

            if (MatchManager.DiceResult == 6)
                Sound.PlaySound(Constants.Instance.sfxLocalPlayer6);
